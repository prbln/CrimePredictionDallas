# -*- coding: utf-8 -*-
"""ML_Project_Division.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/12EsMVtVNL98gUJ-Amb8PiuZrSu3qsWTU
...
"""##Pre-procesing with only Beats"""

df['City'].value_counts()

column_names = ['NIBRS Group' , 'Beat', 'Date of Report', 'Special Report (Pre-RMS)']
#df = df[df['Date of Report'].dt.year >= 2014]
...
new_df = new_df.dropna(subset=['NIBRS Group','Beat'])
...
#Final cleaned Dataset Length
len(new_df)

#Encoding the columns
new_df['NIBRS Group'].value_counts()
...
#removing all rows where year is < 2014
...
# ****************************************************** NIBRS NULL VALUES PER YEAR******************************************
# null_counts_by_year = df.groupby('Year of Incident')['NIBRS Group'].apply(lambda x: x.isnull().sum())
# print(null_counts_by_year)

#MODEL TRAINING

# 1. Combine B,C as Not severe mark 0 ie. not severe, Make A as severe  seveere mark 1 | # y-label nibris group sever or not severe
# 2. Given BEAT, year, month, date, (Watch2) | columns: name - SPECIAL REPORT special day - 1, normal day - 0 | Gender 

# ************************************uss beat me severe crime hoga ya nahi

top_beats = new_df['Beat'].value_counts().head(10)
top_beats_sum = top_beats.sum()
print("Top 10 beats and their count:")
print(top_beats)
print("Sum of top 10 beat counts:", top_beats_sum)

(79509/800495)*100

# Get the top 20 values in the 'Beat' column
top_beats = new_df['Beat'].value_counts().head(10).index.tolist()

# Filter new_df based on the top 20 values
filtered_df = new_df[new_df['Beat'].isin(top_beats)].copy()

filtered_df['Beat'].value_counts()

#Checking Null Values
null_counts = filtered_df.isnull().sum()
null_percentages = null_counts / len(filtered_df) * 100
...
# Convert 'Beat' column from float to int
filtered_df['Beat'] = filtered_df['Beat'].astype(int)
...
nb = GaussianNB(var_smoothing=1e-09, priors=[0.3, 0.7])
...
filtered_df['NIBRS Group'].value_counts()
...
column_names = ['NIBRS Group' , 'Division', 'Date of Report', 'Special Report (Pre-RMS)','Target Area Action Grids','Weapon Used','Victim Type','Victim Race',
...
new_df = new_df.dropna(subset=['NIBRS Group','Division'])
...
new_df['Division'].value_counts()

# Convert all values in "Division" column to lowercase
new_df["Division"] = new_df["Division"].str.lower()
new_df["Division"].value_counts()
...
filtered_df['Division'].value_counts()
...
print("Training Classification Report:",classification_report_train)

classification_report_val = classification_report(y_val, y_pred_val)
print("Validation Classification Report:",classification_report_val)

classification_report_test = classification_report(y_test, y_pred_test)
print("Testing Classification Report:",classification_report_test)
...
classification_report_train = classification_report(y_train, y_pred_train)
print("\nTraining Classification Report:",classification_report_train)

classification_report_val = classification_report(y_val, y_pred_val)
print("\nValidation Classification Report:",classification_report_val)

classification_report_test = classification_report(y_test, y_pred_test)
print("\nTesting Classification Report:",classification_report_test)
...
# Select features with importance above the 50% quantile
...
ax.barh(range(x_train.shape[1]), importances[indices], color=cmap(np.linspace(0, 1, x_train.shape[1])))
...
"""**Now using the Top 6 Models & drawing the Precision Recall Graph**
